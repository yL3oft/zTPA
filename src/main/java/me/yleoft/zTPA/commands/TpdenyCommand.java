...
public class TpdenyCommand extends TpaUtils implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender s, @NotNull Command cmd, @NotNull String label, String[] args) {
        ...
        Player p = (Player) s;
        if (!p.hasPermission(CmdTpdenyPermission())) {
            cmdm.sendMsg(p, cmdm.getNoPermission());
            return false;
        }
        LanguageUtils.Tpdeny lang = new LanguageUtils.Tpdeny();
    ...
    public void code(Player p, Player target, LanguageUtils.Tpdeny lang, LanguageUtils.CommandsMSG cmdm) {
        ...
        for(TeleportRequest request : requests) {
            if (request.getSenderUUID().equals(target.getUniqueId())) {
                if (cfguExtras.canAfford(p, CmdTpdenyPermission(), CmdTpdenyCost())) {
                    request.denyRequest();
                }
                return;
            }
