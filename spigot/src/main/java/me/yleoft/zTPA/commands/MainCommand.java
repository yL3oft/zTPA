...
import static me.yleoft.zTPA.zTPABukkit.checker;
...
public class MainCommand extends ConfigUtils implements CommandExecutor {
        ...
        switch (subcmd) {
            ...
            case "ver":
                ...
                if(args.length >= 2) {
                    ...
                    switch (subcmd2) {
                        ...
                        case "update": {
                            ...
                            try {
                                checker.update();
                                lang.sendMsg(s, langvu.getOutput(zTPABukkit.getInstance().updateVersion));
                            }catch (Exception e) {
                                e.printStackTrace();
                                return false;
                            }
                ...
                if(needsUpdate) {
                    ...
                    cmdm.sendMsg(s, "%prefix%&6New version: &a" + zTPABukkit.getInstance().updateVersion);
                    cmdm.sendMsg(s, "%prefix%&6Your version: &c" + zTPABukkit.getInstance().getDescription().getVersion());
                    cmdm.sendMsg(s, "%prefix%&6You can update your plugin here: &e" + zTPABukkit.getInstance().site);
                }
                return false;
        }
        lang.sendMsg(s, getUsage(s, lang4));
    ...
    public long reload(String which) {
        ...
        switch (which) {
            ...
            case "commands":
                reload("config");
                zTPABukkit.getInstance().loadCommands();
                PluginYAMLManager.syncCommands();
                break;
            case "config":
                zTPABukkit.getInstance().reloadConfig();
                zTPABukkit.cfgu = new ConfigUtils();
                break;
            case "languages":
