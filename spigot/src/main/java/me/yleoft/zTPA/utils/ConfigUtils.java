...
public class ConfigUtils {
    ...
    public static class ConfigUtilsExtras {
        ...
        public boolean canAfford(Player p, String commandPermission, Float cost) {
            if(p.hasPermission(zTPABukkit.cfgu.PermissionBypassCommandCost(commandPermission))) {
                return true;
            }
            Economy economy = (Economy) zTPABukkit.economy;
            LanguageUtils.HooksMSG hooks = new LanguageUtils.HooksMSG();
            if (zTPABukkit.getInstance().getServer().getPluginManager().isPluginEnabled("Vault")) {
